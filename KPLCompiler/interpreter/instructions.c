...
int emitCode(CodeBlock *codeBlock, enum OpCode op, WORD p, WORD q)
{
  ...
  if (codeBlock->codeSize >= codeBlock->maxSize)
    return 0;
...
void printInstruction(Instruction *inst)
{
  ...
  {
  case OP_LA:
    printf("LA %d,%d", inst->p, inst->q);
    break;
  case OP_LV:
    printf("LV %d,%d", inst->p, inst->q);
    break;
  case OP_LC:
    printf("LC %d", inst->q);
    break;
...
void sprintInstruction(char *s, Instruction *inst)
{
  ...
  {
  case OP_LA:
    sprintf(s, "LA %d,%d", inst->p, inst->q);
    break;
  case OP_LV:
    sprintf(s, "LV %d,%d", inst->p, inst->q);
    break;
  case OP_LC:
    sprintf(s, "LC %d", inst->q);
    break;
  case OP_LI:
    sprintf(s, "LI");
    break;
  case OP_INT:
    sprintf(s, "INT %d", inst->q);
    break;
  case OP_DCT:
    sprintf(s, "DCT %d", inst->q);
    break;
  case OP_J:
    sprintf(s, "J %d", inst->q);
    break;
  case OP_FJ:
    sprintf(s, "FJ %d", inst->q);
    break;
  case OP_HL:
    sprintf(s, "HL");
    break;
  case OP_ST:
    ...
    break;
  case OP_CALL:
    sprintf(s, "CALL %d,%d", inst->p, inst->q);
    break;
  case OP_EP:
    sprintf(s, "EP");
    break;
