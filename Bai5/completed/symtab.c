...
Object *lookupObject(char *name)
{
  ...
  // [ADDED] Tìm kiếm đối tượng trong chuỗi scope từ hiện tại đến toàn cục
  // Bắt đầu từ scope hiện tại
  scope = symtab->currentScope;
  while (scope != NULL)
  {
    // [ADDED] Tìm trong scope hiện tại
    obj = findObject(scope->objList, name);
    if (obj != NULL)
    {
      return obj;
    }
    // [ADDED] Di chuyển đến parent scope (scope bên ngoài)
    scope = scope->outer;
  }

  // [ADDED] Nếu không tìm thấy trong chuỗi scope, tìm trong danh sách toàn cục
  ...
  // [ADDED] Không tìm thấy đối tượng
  return NULL;
}
